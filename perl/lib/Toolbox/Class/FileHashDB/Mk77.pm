...
our $VERSION = 'v1.0.4';
##~ DIGEST : 1f52affb1c255ff67f8099ff5d1ed9f1
use Moo;
with(
    qw/
      Moo::Role::DB
...
sub storepath {
    ...
    my ( $name, $path, $suffix ) =
      File::Basename::fileparse( $fullpath, qr/\.[^.]*/ );
...
sub initdirweight {
    my ( $self, $clv ) = @_;
    my $dirliststh =
      $self->dbh->prepare("select * from dir_list order by name asc");
    my $setweightsth =
      $self->dbh->prepare("update dir_list set weight = ? where id = ?");
    $dirliststh->execute();
...
sub setonetrue {
    ...
    while ( my $md5row = $md5ssth->fetchrow_hashref ) {
        ...
        if ( $self->debug() || $clv->{vocal} ) {
            ...
            $self->debug_msg(
"Setting $onetruerow->{id} as one true file for [<unknown digest>]"
            );
        }
...
sub dodeletes {
    ...
    $clv ||= {};
    my $fetchsth =
      $self->dbh->prepare( $self->_path_qstring . "where todelete = 1" );
        ...
        if ( -f $path ) {
            ...
            if ( -e $path ) {
                ...
                if ( $clv->{vocal} ) {
                    ...
                    $self->debug_msg(
                        "Attempted to delete non-existant path [$path]");
                }
