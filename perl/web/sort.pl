...
##~ DIGEST : 2427474ea0bb5e28c7017629998f6bd3
use strict;
...
sub kamove {
    ...
    if ( index( $target, '.' ) != -1 ) {
        confess(
"Trying to do something with relative or hidden target paths [$target] "
        );
    }
    $target =~ s|/||g;
    ...
    if ($exists) {
        my ( $name, $path, $suffix ) =
          File::Basename::fileparse( $actual_file, qr/\.[^.]*/ );
        require Data::UUID;
        my $ug         = Data::UUID->new;
    ...
    if ( exists( $map->{replace} ) ) {
        for my $key ( keys( %{ $map->{'map'} } ) ) {
            for my $replace ( keys( %{ $map->{replace} } ) ) {
                $map->{'map'}->{$key} =~
                  s|$replace|$map->{replace}->{$replace}|g;
                $map->{'map'}->{$key} .= $extra;
            }
        }
        $map = $map->{'map'};
