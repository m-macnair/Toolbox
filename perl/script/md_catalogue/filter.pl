...
STHS: {
    for my $pair (
        ...
        [
            'sth_get_hashes',
            'select distinct(md5) from file_list where md5 is not null'
        ],
        [
            'sth_delete_hash',
...
sub main {
    ...
    my $clv = Toolbox::CombinedCLI::get_config( [qw/rawdb filterdbs /],
        [qw/ dodeletes vocal /] );
    ...
    for my $filterdb ( @{$filterfiles} ) {
        ...
        while ( my $filter_row =
            $mk77_filter->sth_get_hashes()->fetchrow_arrayref() )
        {
