...
sub hreftocsv {
	...
	my $columns = $self->{instance}->{data}->{'Toolbox::CSV'}->{$filename}->{columns};
	...
	unless ( $columns ) {
		...
		$self->csv->print( $self->ofh_for( $filename ), $columns ) unless $conf->{noheadings};
		$self->{instance}->{data}->{'Toolbox::CSV'}->{$filename}->{columns} = $columns;
...
sub csv {
	...
	return $self->lload(
		{
			initsub => sub { "Text::CSV"->new( {binary => 1, eol => "\n"} ); },
			%{$p},
			module => "Text::CSV"
		}
	);
