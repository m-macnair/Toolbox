...
sub _init {
	my ( $self, $args ) = @_;
	confess( "Missing the parent Toolbox::ExcelWriterSugar object" ) unless ( ref( $args->{ews} ) eq 'Toolbox::ExcelWriterSugar' );
	$self->ews( $args->{ews} );
	my $current_work_sheet = $args->{wso} || $self->ews()->ewx()->add_worksheet();
