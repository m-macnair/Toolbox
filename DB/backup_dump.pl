  ...
  DUMPPREFIX: {
    ...
    for my $table (@tables) {
        ...
        unless ( $C->{skip_schema} ) {
            my $newfile =
              abspath( "$table_dir/schema_[" . time_string() . '].sql' );
            my $cmd =
"mysqldump $dumpprefix --single-transaction --no-data -u $C->{user} -p$C->{pass} $C->{db} $table";
            print ` $cmd > $newfile`;
      ...
      DATA: {
            my $newfile =
              abspath( "$table_dir/data_[" . time_string() . '].sql' );
            my $cmd =
"mysqldump $dumpprefix --single-transaction --no-create-info -u $C->{user} -p$C->{pass} $C->{db} $table";
            print ` $cmd > $newfile`;
    ...
    subonfiles(
        sub {
            ...
            if ( index( $oldfile, $string ) != -1 ) {
                ...
                print
                  "Checking $oldfile.$olddigest against $newfile.$newdigest $/";
